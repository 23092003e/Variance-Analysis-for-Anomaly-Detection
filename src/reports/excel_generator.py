...
class ExcelGenerator:
    ...
    def _create_anomalies_summary_sheet(self, writer: pd.ExcelWriter, anomalies: List[Anomaly]) -> None:
        """Create the main Anomalies Summary sheet."""
        if not anomalies:
            self.logger.warning("No anomalies found - creating empty summary sheet")
        
        # Prepare data for anomalies summary
        summary_data = []
        for anomaly in anomalies:
            summary_data.append({
                ...
                'Account Name': anomaly.account_name,
                ...
                'Variance %': anomaly.variance_percent,
                'Rule Violated': anomaly.rule_violated or '',
        ...
        df_summary.to_excel(writer, sheet_name=sheet_name, index=False, startrow=2)
        ...
        worksheet.merge_range('A2:L2', f'Generated on: {timestamp}', subtitle_format)
        
        # Apply conditional formatting
        if len(df_summary) > 0:
            self.formatter.apply_anomaly_formatting(worksheet, df_summary, start_row=3)
